...
namespace Assets.code
{
    ...
    class Hexapod
    {
        ...
        {
            // Detecta si el hexápodo choca con un cubo
        ...
        public void MoveHexapodBodyDir(float step, Direction dir)
        {
            ...
            // Calcula los pasos en las direcciones X e Y basados en la dirección y la rotación actual
            ...
            // Raycast para detectar obstáculos en la dirección del movimiento
            RaycastHit hit;
            Vector3 rayOrigin = hexapod.transform.position + Vector3.up * 0.3f; // Origen elevado
            Vector3 rayDirection = new Vector3(stepX, stepY, 0).normalized; // Dirección del movimiento

            // Dibuja el raycast en la escena para verificarlo visualmente
            Debug.DrawRay(rayOrigin, rayDirection * step, Color.red, 1.0f);

            if (Physics.Raycast(rayOrigin, rayDirection, out hit, step))
            {
                Debug.Log($"Hit detected: {hit.collider.name} at {hit.point}");
                if (hit.collider.CompareTag("Obstacle"))
                {
                    hexapod.transform.localPosition = new Vector3(
                        hexapod.transform.localPosition.x + stepX,
                        hexapod.transform.localPosition.y + stepY,
                        hit.point.z + 0.5f // Ajuste rápido al obstáculo
                    );
                }
            }
            else
            {
                ...
                hexapod.transform.localPosition = new Vector3(
                    hexapod.transform.localPosition.x + stepX,
                    hexapod.transform.localPosition.y + stepY,
                    hexapod.transform.localPosition.z
                );
            }
            ...
            Parameters.bodyLocalPosition = new MyVector3(
                Parameters.bodyLocalPosition.x + stepX,
                Parameters.bodyLocalPosition.y + stepY,
                Parameters.bodyLocalPosition.z
            );
        }

        // Método para ajustar la altura del hexápodo basado en la posición de las patas
        private void AdjustHeight()
        {
            ...
            {
                RaycastHit legHit;
                Vector3 legPosition = leg.alpha.position; // Usa la posición de la pata como origen del raycast

                // Raycast hacia abajo desde cada pata
                if (Physics.Raycast(legPosition, Vector3.down, out legHit, 2.0f))
                {
                    Debug.Log($"Leg {leg.name} detected ground at {legHit.point.z}");
                    maxHeight = Mathf.Max(maxHeight, legHit.point.z);
                }
            }

            // Ajusta la posición del cuerpo del hexápodo
            if (maxHeight > float.MinValue)
            {
                hexapod.transform.localPosition = new Vector3(
                    hexapod.transform.localPosition.x,
                    hexapod.transform.localPosition.y,
                    maxHeight + 0.1f // Ajusta ligeramente por encima del terreno detectado
                );
            }
        }
